import { mappingPostData, mappingPostDetail } from "../../helpers";
...
export function actFetchPostDetail(detail) {
  return {
...
export function actFetchArticleGeneralAsync({
  ...
  return async (dispatch) => {
    try {
      ...
      const response = await postService.getArticleGeneral({ perPage, currentPage, keySearch, categoriesId});
...
export function actFetchPostDetailAsync(slug) {
  return async (dispatch) => {
    try {
      ...
      const posts = response.data.map(mappingPostDetail)
      dispatch(actFetchPostDetail(posts[0]))
    } catch (err) {
      // TODO 
    }
  }
